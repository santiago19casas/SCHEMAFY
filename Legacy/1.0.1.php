...
define('BASEO_SCHEMA_VERSION', '1.0.0');
...
class CustomSchemaByBaseo {
    ...
    public function enqueue_admin_scripts($hook) {
        ...
        wp_enqueue_script('baseo-schema-admin', BASEO_SCHEMA_PLUGIN_URL . 'admin.js', array('jquery'), BASEO_SCHEMA_VERSION, true);
        wp_enqueue_style('baseo-schema-admin', BASEO_SCHEMA_PLUGIN_URL . 'admin.css', array(), BASEO_SCHEMA_VERSION);
                    ...
                    <div class="baseo-panel baseo-schemas-list">
                        <div class="baseo-panel-header">
                            ...
                            <div class="baseo-panel-actions">
                                <input type="text" id="baseo-search" placeholder="🔍 <?php _e('Search by URL...', 'custom-schema-baseo'); ?>" class="baseo-search" />
                                <button id="baseo-refresh" class="baseo-btn-icon">🔄</button>
                            </div>
                        </div>
    ...
    public function add_frontend_editor() {
        if (!is_admin() && current_user_can('manage_options')) {
                    ...
                    <div class="baseo-editor-content">
                        <div class="baseo-editor-info">
                            ...
                            <div class="baseo-existing-list">
                                <h4>📋 <?php printf(__('Existing Schemas (%d):', 'custom-schema-baseo'), count($existing_schemas)); ?></h4>
                                <?php foreach ($existing_schemas as $schema): ?>
                                <div class="baseo-mini-item" style="background: #f8f9fa; padding: 8px 12px; margin: 4px 0; border-radius: 6px; display: flex; justify-content: space-between; align-items: center;">
                                    <div>
                                        <span><?php echo $schema['is_active'] ? '✅' : '⏸️'; ?> <?php echo esc_html($schema['schema_name']); ?></span>
                                        <small style="color: #666; margin-left: 8px;"><?php echo $this->get_schema_icon($schema['schema_type']); ?> <?php echo $schema['schema_type']; ?></small>
                                    </div>
                                    <button class="baseo-btn-tiny baseo-delete-existing" data-id="<?php echo $schema['id']; ?>" style="background: #e74c3c; color: white; border: none; padding: 2px 6px; border-radius: 3px; font-size: 10px; cursor: pointer;">🗑️</button>
                                </div>
                                <?php endforeach; ?>
                            </div>
                            <?php endif; ?>
                        ...
                        <form id="baseo-frontend-form">
                            ...
                            <div class="baseo-editor-actions">
                                ...
                                <button type="submit" class="baseo-btn-primary">💾 <?php _e('Save Schema', 'custom-schema-baseo'); ?></button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            document.getElementById('baseo-frontend-form').addEventListener('submit', function(e) {
                    ...
                    if (xhr.readyState === 4) {
                        if (xhr.status === 200) {
                            try {
                                ...
                                if (response.success) {
                                    alert('✅ Schema "' + schemaName + '" saved successfully!');
                                    baseoToggleSchemaEditor();
                                    location.reload(); // Reload to see updated button
                                } else {
                ...
                var data = 'action=baseo_save_schema&nonce=<?php echo wp_create_nonce('baseo_schema_nonce'); ?>&url=' + encodeURIComponent(url) + '&schema_name=' + encodeURIComponent(schemaName) + '&schema_data=' + encodeURIComponent(schemaData) + '&schema_type=' + encodeURIComponent(schemaType);
                xhr.send(data);
            });
