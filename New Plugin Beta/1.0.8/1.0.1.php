...
class CustomSchemaByBaseo {
    ...
    public function add_plugin_links($links) {
        $custom_links = array(
            '<a href="' . admin_url('admin.php?page=baseo-custom-schema') . '" style="color: ' . $this->brand_color . '; font-weight: bold;">⚙️ ' . __('Configure', 'custom-schema-baseo') . '</a>',
...
public function add_admin_menu() {
    $page = add_menu_page(
        ...
        'dashicons-media-code',  // ← ICONO (puedes cambiarlo)
        30  // ← POSICIÓN en el menú (30 = después de "Comentarios")
    );
    ...
    wp_enqueue_style(
        'baseo-schema-admin', 
        BASEO_SCHEMA_PLUGIN_URL . 'assets/css/admin.css?v=' . time(), 
        array(), 
        null,
        'all'
    );
    ...
    public function render_schema_meta_box($post) {
        ...
        <div class="baseo-meta-box">
            ...
            <div class="baseo-manage-link">
                <a href="<?php echo admin_url('admin.php?page=baseo-custom-schema&url=' . urlencode($current_url)); ?>" 
    ...
    public function add_admin_bar_button($wp_admin_bar) {
        if (!is_admin() && current_user_can('manage_options')) {
            ...
            $wp_admin_bar->add_node(array(
                ...
                'title' => $button_text,
                'href'  => admin_url('admin.php?page=baseo-custom-schema&url=' . urlencode($current_url)),
                'meta'  => array(
                    'target' => '_blank'
                )
            ));
