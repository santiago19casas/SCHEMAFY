...
jQuery(document).ready(function($) {
    ...
    function getBulkSchemaPanel() {
        ...
        // RIGHT: URL Selector
             html += '<div>';
            html += '<div class="baseo-panel-header">';
            html += '<div style="display: flex; justify-content: space-between; align-items: center; gap: 20px; margin-bottom: 20px;">';
            html += '<h3 style="margin: 0;">📂 ' + t('select_urls', 'Select URLs') + ' <span id="baseo-bulk-url-count">0</span></h3>';
            html += '<input type="text" id="baseo-bulk-search-urls" class="baseo-input" style="width: 320px; margin: 0; padding: 12px 20px;" placeholder="🔍 ' + t('search_schemas', 'Search URLs...') + '" />';
            html += '</div>';
...
$(document).on('click', '.baseo-edit-schema', function() {
    ...
    // BULK: Search URLs
    $(document).on('input', '#baseo-bulk-search-urls', function() {
        var searchQuery = $(this).val().trim();
        window.BASEO.filterUrlsBySearch(searchQuery);
    });
