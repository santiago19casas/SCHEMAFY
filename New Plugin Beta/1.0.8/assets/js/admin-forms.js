...
(function($) {
    ...
    var html = '';  // ← SIN wrapper (ya existe en HTML)
    ...
    urls.forEach(function(urlData) {
        ...
        html += '</div>'; // .baseo-url-meta
        html += '</div>'; // .baseo-url-info
        html += '</div>'; // .baseo-url-item
    });
...
/**
     ...
     */
    window.BASEO.filterUrlsBySearch = function(searchQuery) {
        window.BASEO.bulkState.currentSearch = searchQuery.toLowerCase();
        applyBulkFilters();
    };
    ...
    function applyBulkFilters() {
        var type = window.BASEO.bulkState.currentFilter;
        var search = window.BASEO.bulkState.currentSearch || '';
        
        $('.baseo-url-item').each(function() {
            var $item = $(this);
            var itemType = $item.data('type');
            var title = $item.find('.baseo-url-title').text().toLowerCase();
            var path = $item.find('.baseo-url-path').text().toLowerCase();
            
            var typeMatch = (type === 'all' || itemType === type);
            var searchMatch = (search === '' || title.includes(search) || path.includes(search));
