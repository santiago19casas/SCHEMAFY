...
jQuery(document).ready(function($) {
    ...
    function renderAdminUI(skipInitialLoad = false) {
        ...
        // Meta Title field
        html += '<div class="baseo-form-group">';
        html += '<label for="baseo-meta-title" class="baseo-label">';
        html += '📝 ' + t('meta_title', 'Meta Title') + ' <span class="baseo-optional">(Optional)</span>';
        html += '</label>';
        html += '<input type="text" id="baseo-meta-title" name="meta_title" class="baseo-input" maxlength="70" />';
        html += '<div class="baseo-char-counter baseo-counter-optimal" id="baseo-meta-title-counter">';
        html += '<span class="baseo-char-count">0</span>/<span class="baseo-char-limit">70</span> - ';
        html += '<span class="baseo-char-remaining">70 ' + t('chars_left', 'characters left') + '</span>';
        html += '</div>';
        html += '</div>';
        
        // Meta Description field
        html += '<div class="baseo-form-group">';
        html += '<label for="baseo-meta-description" class="baseo-label">';
        html += '📄 ' + t('meta_description', 'Meta Description') + ' <span class="baseo-optional">(Optional)</span>';
        html += '</label>';
        html += '<textarea id="baseo-meta-description" name="meta_description" rows="3" class="baseo-input" maxlength="160"></textarea>';
        html += '<div class="baseo-char-counter baseo-counter-optimal" id="baseo-meta-description-counter">';
        html += '<span class="baseo-char-count">0</span>/<span class="baseo-char-limit">160</span> - ';
        html += '<span class="baseo-char-remaining">160 ' + t('chars_left', 'characters left') + '</span>';
        html += '</div>';
        html += '</div>';
        ...
        $('#baseo-meta-title').on('input', function() {
            updateCharCounter(this, 'baseo-meta-title-counter', 70);
        });
        
        $('#baseo-meta-description').on('input', function() {
            updateCharCounter(this, 'baseo-meta-description-counter', 160);
        });
    ...
    $(document).on('click', '.baseo-variable-item', function() {
        ...
        if (navigator.clipboard) {
            navigator.clipboard.writeText(variable).then(function() {
                showNotification(t('variable_copied', '✅ Variable copied!'), 'success');
            });
        } else {
    ...
    // Close variables dropdown when clicking outside
    $(document).on('click', function(e) {
        if (!$(e.target).closest('#baseo-insert-variables, #baseo-variables-dropdown').length) {
            $('#baseo-variables-dropdown').hide();
        }
    });
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    if (isEdit) {
                        ...
                        updateCharCounter('#baseo-meta-title', 'baseo-meta-title-counter', 70);
                        updateCharCounter('#baseo-meta-description', 'baseo-meta-description-counter', 160);
                    }
                    loadSchemas(currentPage, false);
                } else {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $('#baseo-meta-title').val(schema.meta_title || '');
                    $('#baseo-meta-description').val(schema.meta_description || '');
